...
data<-read.csv("C:/Users/tbaidoo/Desktop/Research/SEER Breast Cancer Dataset .csv")
...
data$age_group <- cut(data$Age,
                      breaks = c(30, 50, 60, 70),
                      labels = c("30-50", "50-60", "60-70"),
                      right = FALSE)  # "right = FALSE" ensures left-closed intervals
...
data$tumor_size_group <- cut(data$Tumor.Size,
                             breaks = c(-Inf, 40, 65, Inf),
                             labels = c("<40mm", "40-65mm", ">65mm"),
                             right = FALSE)


table(data$tumor_size_group)
...
fit_tumor <- survfit(Surv(Survival.Months, Status) ~ tumor_size_group, data = data)
...
pairwise_comp_tumor <- pairwise_survdiff(Surv(Survival.Months, Status) ~ tumor_size_group, data = data, p.adjust.method = 'bonferroni')
...
# Stratifying E Status, P Status and A Stage
